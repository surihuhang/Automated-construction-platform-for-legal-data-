...
with col1:
    ...
    if uploaded_file is not None:
        ...
        if current_file_name != st.session_state.processed_file_name:
            try:
                ...
                if extracted_text.strip():
                    ...
                    # 清除 source_text_area 的 key，确保文本框能显示新内容
        ...
        # 如果没有上传文件，清除已处理文件的标记（允许重新上传）
        if st.session_state.processed_file_name:
            st.session_state.processed_file_name = ""
    
    # 文本输入框（使用 session_state 的值，确保文件上传后能正确显示）
    # 使用动态 key 确保文件上传后能正确更新显示
    text_area_key = f"source_text_area_{st.session_state.processed_file_name}" if st.session_state.processed_file_name else "source_text_area"
